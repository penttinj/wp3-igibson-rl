...
class Wp3TestEnv(BaseEnv):
    ...
    def load_task_setup(self):
        ...
        elif self.config["task"] == "static_goal":
            self.task = StaticGoalTask(self)
        elif self.config["task"] == "go_to_object":
            self.task = GoToObjectTask(self)
        else:
            try:
                import bddl

                with open(
                    os.path.join(os.path.dirname(bddl.__file__), "activity_manifest.txt")
                ) as f:
