# Forked from iGibson project by Stanford Vision and Learning Group
# https://github.com/StanfordVL/iGibson/blob/master/igibson/sensors/vision_sensor.py
import os
...
from igibson.sensors.object_recognition import ObjectRecognition


class VisionSensor(BaseSensor):
    """
    Vision sensor (including rgb, rgb_filled, depth, 3d, seg, normal, optical flow, scene flow)
    Modified to include a "sensor" which recognizes objects within the rgb frame
    """
    ...
    def __init__(self, env, modalities):
        ...
        if "recognition" in modalities:
            self.image_classifier = ObjectRecognition()
    ...
    def get_raw_modalities(self, modalities):
        ...
        if (
            "rgb" in modalities
            or "rgb_filled" in modalities
            or "highlight" in modalities
            or "recognition" in modalities
        ):
    ...
    def get_obs(self, env) -> OrderedDict:
        """
        ...
        if "recognition" in self.modalities:
            rgb = self.get_rgb(raw_vision_obs)
            vision_obs["recognition"] = self.image_classifier.classify(rgb)
